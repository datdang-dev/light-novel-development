...
| -------------------- | ----------------------------------------------- | ----------------------------------------------------------------------------------------------- |
| **Encryption Analysis** | "analyze game", "check encryption", "pak format" | Load `{project-root}/studio/modules/lnd-re-specialist/references/encryption-analysis.md` |
| **AES RAM KPA**      | "find aes key", "extract key", "ram kpa", "bypass encryption" | Load `{project-root}/studio/modules/lnd-re-specialist/references/aes-ram-kpa.md`         |
| **Asset Workflow**   | "extract models", "fmodel", "blender import", "export assets" | Load `{project-root}/studio/modules/lnd-re-specialist/references/asset-workflow.md`     |
...
Scripts are located in `{project-root}/studio/modules/lnd-re-specialist/scripts/` and use standard Python 3 with `pycryptodome` for AES decryption and `ctypes` for Win32 API memory reading.
