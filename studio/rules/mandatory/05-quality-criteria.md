...
### Gate 0.5: Anti-Slop / Neutral Narrator (HARD FAIL)

**This gate runs AFTER format check, BEFORE scoring. If ANY item fails → score = 0, return to prose-adapter.**

Scan ALL narrator text (excluding character dialogue) for the following banned patterns:
- [ ] No moral judgment adjectives? (bệnh hoạn, gớm ghiếc, kinh tởm, đê hèn, nhục nhã)
- [ ] No moralizing labels? (tha hóa, sa ngã, vứt bỏ liêm sỉ, mất nhân tính, tội lỗi)
- [ ] No editorial commentary? (chà đạp hình tượng, vũng bùn [metaphor], cảnh tượng [judgmental])
- [ ] No narrator sympathy/pity? (tội nghiệp, đáng thương)
- [ ] Narrator voice is PURELY sensory? (actions, fluids, sounds, temperature, texture ONLY)

```
IF ANY ANTI-SLOP ITEM FAILS:
  Score = 0/100
  Verdict = SLOP-FAIL
  Action = Return to Suki with specific offending sentences highlighted
  Reference = lewd_writing_mechanics.md > PILLAR 0: ANTI-SLOP DIRECTIVE
  DO NOT proceed to category scoring
```
