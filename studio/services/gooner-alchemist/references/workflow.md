...
- **Step 3:** Invoke Core Transformation Engine
  ...
  - *JIT Compilation:* Before invoking Suki, compile `{{project_root}}/studio/config/pipeline-context.md` and `{{project_root}}/studio/rules/lewd_writing_mechanics.md` into the agent payload `context_payload.md` to prevent mid-execution disk reads.
...
3. **HORIZON VALIDATION:** The current `context_horizon.md` is a stub placeholder and MUST NOT be treated as factual trajectory evidence. The FORENSIC IMAGE dictates current Ground Truth.
4. **ACTION DEDUPLICATION:** Auto-deduplication is currently disabled because the horizon generator is not implemented. Do not merge pages based on stub horizon output.
