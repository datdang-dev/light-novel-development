...
stateTemplate: '{{run_dir}}/_templates/pipeline-state-template.yaml'
...
**{{project_name}}** - Page {{current_page}} đang xử lý
...
```bash
mkdir -p {{run_dir}}/_pipeline/{{project_name}}
mkdir -p {{run_dir}}/_analysis/{{project_name}}
mkdir -p {{run_dir}}/_prose/{{project_name}}
mkdir -p {{run_dir}}/_bible/{{project_name}}
mkdir -p {{run_dir}}/_pipeline/{{project_name}}/agent-memory
```
