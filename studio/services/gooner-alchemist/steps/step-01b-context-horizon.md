...
Read `{{stateFile}}` (`{{run_dir}}/_pipeline/{{project_name}}/state.yaml`) and extract:
...
IF current_page IS NOT first page AND previous_page state IS MISSING:
  ...
  📋 Missing: {{run_dir}}/_pipeline/{{project_name}}/page-{prev}-state.yaml
...
📁 OUTPUT: {{run_dir}}/{{chapter}}/{{page}}/context_horizon.md
─────────────────────────────────────────────
```
...
# Output: {{run_dir}}/{{chapter}}/{{page}}/context_horizon.md
python3 {{project_root}}/studio/services/gooner-alchemist/tools/generate_horizon.py \
  --state "{{run_dir}}/_pipeline/{{project_name}}/state.yaml" \
  --mode "video" \
  --window 10 \
  --out "{{run_dir}}/{{chapter}}/{{page}}/context_horizon.md"
```
