...
| Module | Path | Purpose |
|--------|------|---------|
| Gooner Audit Engine | `{{project_root}}/studio/modules/gooner-audit-engine/SKILL.md` | 100-point scoring system |
...
Load config from `{{project_root}}/studio/config/config.yaml`:
...
```yaml
input:
  - prose_file: "{{run_dir}}/_prose/{{manga_name}}/chapter_{{ch}}/page_{{page_num}}_prose.md"

output:
  - audit_report: "{{run_dir}}/_prose/{{manga_name}}/chapter_{{ch}}/page_{{page_num}}_audit.md"
```
